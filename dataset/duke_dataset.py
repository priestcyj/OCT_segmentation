...
class DUKE(torch.utils.data.Dataset):
    def __init__(self, config, img_ids, num_classes, lens, mode=None):
        self.config = config
        self.img_ids = img_ids
        if platform.system() == 'Windows':
            self.data_dir = r'D:\My_Project\yf_code\OCT_DATA\DUKE_Processed_by_Dr_Fan_Yang'
        else:
            self.data_dir = r'/data/yf/OCT_DATA/DUKE_Processed_by_Dr_Fan_Yang'
    ...
    def __getitem__(self, idx):
        ...
        prob_labels = F.one_hot(lab, num_classes=self.config['sdf_num_classes']).permute((2, 0, 1)).float()
